...
class Image extends DatabaseObject {
	...
	public static function find_image($property, $property_id=0) {
		$result_array = static::find_by_sql("SELECT * FROM ".self::$table_name." WHERE (property = '{$property}') AND (property_id = {$property_id}) LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false ;
	}
